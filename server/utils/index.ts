...
// initSocket is a function that initializes the Socket.IO server with an incoming HTTP event (H3Event).

export function initSocket(event: H3Event) {

    // io.attach attaches the Socket.IO server to the existing HTTP server.
    // @ts-ignore
    ...
    io.on('connection', (socket: Socket) => {
        ...
        socket.on('joinRom', (payload: User) => {
            ...
            socket.join(user.room); // user has joined room
